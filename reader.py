...
def frame():
    ...
    lable0 = tk.Label(acs, text='读者登录', font=('微软雅黑', 50), fg='GhostWhite', bg='MediumOrchid').pack()
    ...
    tk.Button(acs, text='退 出', font=('黑体', 15), width=10, height=2, fg='GhostWhite', bg='MediumOrchid',command=exit_reader).place(x=550, y=500)
    acs.mainloop()
...
def login():
    ...
    button1 = tk.Button(root1, text='确定', height=2, width=10, command=lambda: idus.id_check('0'))
...
def register():
    ...
    button1 = tk.Button(root2, text='确定', height=2, width=10, command=lambda: idus.id_write('0'))
